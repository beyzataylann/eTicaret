    		...
    		if(veri!=null){
        		...
        		for(urunModel urun:urunler){
        			...
        			if(urun.getKategoriKodu()==kategoriKodu){
                        ...
                        <img src="src/main/webapp/img/<%=urun.getUrunFotograf() %>" class="card-img-top" alt="...">
                        <div class="card-body">
                            <h5 class="card-title"><%=urun.getUrunAdi() %></h5>
                       
                            <span><%=urun.getUrunFiyat() %></span>
                            <a href="Urunler.jsp?urunKodu=<%=urun.getUrunKodu()%>&urunFotograf=<%=urun.getUrunFotograf()%>&urunAdi=<%=urun.getUrunAdi()%>&urunFiyati=<%=urun.getUrunFiyat()%>&k=<%=kategoriKodu%>" class="btn btn-success float-right"><span><img src="../img/sepet.png" alt="" style="width: 25px" class="mr-1"></span>Sepete Ekle</a>
                        </div>
                    </div>
    				<%
        			}
